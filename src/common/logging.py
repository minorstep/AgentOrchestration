...
from src.common.exception_tracking import ExceptionContextSanitizer


class StructuredFormatter(logging.Formatter):
    def __init__(self):
        super().__init__()
        self._exception_sanitizer = ExceptionContextSanitizer()
    ...
    def format(self, record: logging.LogRecord) -> str:
        ...
        if record.exc_info and record.exc_info[0]:
            exception_context = getattr(record, "exception_context", {})
            local_variables = getattr(record, "local_variables", {})
            log_entry["exception"] = self._exception_sanitizer.sanitize(
                record.exc_info[1],
