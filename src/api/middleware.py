...
class AuthMiddleware(BaseHTTPMiddleware):
    def __init__(self, app, service=None):
        super().__init__(app)
        self.service = service or auth_service
    ...
    async def dispatch(
        ...
        if path.startswith("/api/v2") and path != "/api/v2/auth/token":
            workspace_id = self._task_monitor_workspace(path)
            if workspace_id:
                try:
                    request.state.auth_service = self.service
                    request.state.auth_context = self.service.validate_request(
    ...
    @staticmethod
    def _task_monitor_workspace(path: str):
        parts = path.strip("/").split("/")
        if len(parts) >= 6 and parts[:3] == ["api", "v2", "workspaces"]:
            if parts[4] == "task-monitor":
                return parts[3]
        return None
