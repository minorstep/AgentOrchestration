...
service = AgentAPIService(registry)
...
async def list_agents(
    ...
    return {"agents": service.list_agents(status=status, group=group)}
...
async def register_agent(
    ...
    agent_id = service.register_agent(name, agent_type, config)
    return {"agent_id": agent_id, "status": "registered"}


@router.get("/agents/count")
async def agent_count():
    return {"count": service.count_agents()}
