...
class OrchestratorClient:
    ...
    def _resolve_api_key(api_key: Optional[str] = None) -> str:
        candidate = api_key if api_key is not None else os.getenv("AO_API_KEY")
        if candidate is None:
            raise AuthenticationError(
                "AO_API_KEY is required. Pass api_key or set "
                "AO_API_KEY before creating OrchestratorClient."
            )
        if not isinstance(candidate, str):
            raise AuthenticationError(
                "AO_API_KEY must be a non-empty string. Pass api_key or set "
                "AO_API_KEY."
            )
